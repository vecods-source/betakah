...
export default function MemoriesScreen() {
  ...
  const { allEvents, isLoadingAllEvents } = useAppSelector(
    (state) => state.events
  );
  ...
  const getEventTypeConfig = (type: string) => {
    switch (type) {
      case "WEDDING":
        return {
          icon: "ring",
          label: isRTL ? "زفاف" : "Wedding",
          color: "#E91E63",
        };
      case "BIRTHDAY":
        return {
          icon: "cake-variant",
          label: isRTL ? "عيد ميلاد" : "Birthday",
          color: "#FF9800",
        };
      case "GRADUATION":
        return {
          icon: "school",
          label: isRTL ? "تخرج" : "Graduation",
          color: "#4CAF50",
        };
      case "BABY_SHOWER":
        return {
          icon: "baby-carriage",
          label: isRTL ? "استقبال مولود" : "Baby Shower",
          color: "#E91E63",
        };
      case "ENGAGEMENT":
        return {
          icon: "heart-multiple",
          label: isRTL ? "خطوبة" : "Engagement",
          color: "#E91E63",
        };
      default:
        return {
          icon: "calendar-star",
          label: isRTL ? "مناسبة" : "Event",
  ...
  const filters: { key: TimeFilter; label: string; count: number }[] = [
    ...
    },
    {
      ...
      count: categorizedEvents.today.length,
    },
    {
      key: "PAST",
      label: isRTL ? "منتهيه" : "Past",
    ...
    const cardContent = (
      <>
        ...
        <View style={styles.smallContent}>
          <View
            style={[styles.smallTypeDot, { backgroundColor: typeConfig.color }]}
          />
    ...
    return (
      <View style={styles.timelineSection}>
        <View style={styles.timelineHeader}>
          <View
            style={[styles.timelineIcon, { backgroundColor: `${color}15` }]}
          >
            <Feather name={icon as any} size={18} color={color} />
          </View>
          <Text style={[styles.timelineTitle, { color: textPrimary }]}>
            {title}
          </Text>
          <Text style={[styles.timelineCount, { color: textSecondary }]}>
            {events.length}{" "}
            {isRTL ? "مناسبة" : events.length === 1 ? "event" : "events"}
          </Text>
        </View>
        ...
        {events.length > 1 && (
          <View style={styles.smallGrid}>
            {events
              .slice(1, 5)
              .map((event, index) => renderSmallEvent(event, index))}
          </View>
        )}

        {events.length > 5 && (
          <TouchableOpacity
            style={[styles.viewMoreButton, { backgroundColor: cardBackground }]}
          >
            <Text style={[styles.viewMoreText, { color: Colors.primary }]}>
              {isRTL
                ? `عرض ${events.length - 5} المزيد`
                : `View ${events.length - 5} more`}
            </Text>
            <Feather
          ...
          <TouchableOpacity
            ...
            style={[
              ...
              activeFilter === filter.key && {
                backgroundColor: isDark ? colors.gray[900] : colors.black,
              },
            ]}
